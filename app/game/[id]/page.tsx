...
import Header from "@/components/Header"; // Import Header dùng chung
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-black flex flex-col font-sans">
      <main className="max-w-[1200px] mx-auto w-full p-4 sm:p-8 flex-1">
        ...
        {/* Text nhỏ điều hướng (Breadcrumb) */}
        ...
        <div className="flex items-center gap-4 mb-10 pb-6 border-b border-zinc-200 dark:border-zinc-800">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-sm">Kho tài khoản cực phẩm, uy tín</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {game.accounts.map((acc) => (
            <div 
              key={acc.id} 
              className="bg-white dark:bg-zinc-900 rounded-3xl overflow-hidden border border-zinc-200 dark:border-zinc-800 flex flex-col shadow-sm hover:shadow-xl transition-all group"
            >
              {/* Thumbnail ACC */}
              ...
              <div className="p-4 flex flex-col flex-1">
                <h3 className="font-bold text-zinc-900 dark:text-zinc-100 line-clamp-2 mb-2 min-h-[3rem]">
                  {acc.name}
                </h3>
                ...
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-zinc-100 dark:border-zinc-800">
                 ...
                 <Link 
  href={`/account/${acc.id}`} // Đường dẫn tới trang chi tiết mới tạo
  className={`p-3 rounded-2xl transition-all ${
    acc.status === 'conhang' 
    ? 'bg-[#f58220] text-white hover:bg-[#e0761d] active:scale-95' 
