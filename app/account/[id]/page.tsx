...
// 1. Skeleton Loading cho trang chi tiết ACC
    ...
    <div className="animate-pulse space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-7 space-y-4">
          <div className="aspect-video w-full bg-zinc-200 dark:bg-zinc-800 rounded-3xl" />
        </div>
        <div className="lg:col-span-5 space-y-6">
          <div className="h-[300px] w-full bg-zinc-200 dark:bg-zinc-800 rounded-3xl" />
          <div className="h-[150px] w-full bg-zinc-200 dark:bg-zinc-800 rounded-3xl" />
        </div>
      </div>
    </div>
  );
...
// 2. Nội dung chính xử lý Logic Fetch Data
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-2 text-[11px] sm:text-xs text-zinc-500 mb-6">
        ...
        <Link href={`/game/${account.gameId}`} className="hover:text-[#f58220] transition-colors">
          {account.game.name}
        </Link>
        <ChevronRight size={12} />
          ...
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3 mb-8">
              <div className="flex items-center gap-2 p-3 bg-zinc-50 dark:bg-zinc-800 rounded-2xl border border-zinc-100 dark:border-zinc-700/50">
                ...
                <span className="text-[11px] font-bold dark:text-zinc-300 uppercase">Uy tín 100%</span>
              </div>
              <div className="flex items-center gap-2 p-3 bg-zinc-50 dark:bg-zinc-800 rounded-2xl border border-zinc-100 dark:border-zinc-700/50">
                <Zap className="text-yellow-500" size={20} />
                <span className="text-[11px] font-bold dark:text-zinc-300 uppercase">Giao ngay</span>
              </div>
            </div>
            ...
            <a
              ...
              className="flex items-center justify-center gap-3 w-full bg-[#0068ff] hover:bg-[#0056d2] text-white py-4 rounded-2xl font-black text-lg transition-all active:scale-95 shadow-lg shadow-blue-500/20"
            >
              <MessageCircle size={24} />
              MUA NGAY QUA ZALO
            </a>
            <p className="text-center text-[11px] text-zinc-400 mt-4 italic">
              * Nhấn để chat trực tiếp với Admin nhận tài khoản
            </p>
          </div>

          {/* Chi tiết mô tả */}
          <div className="bg-white dark:bg-zinc-900 p-6 rounded-3xl border border-zinc-200 dark:border-zinc-800">
            <h2 className="font-bold text-zinc-900 dark:text-white mb-4 border-l-4 border-[#f58220] pl-3 uppercase text-sm tracking-widest">
              Thông tin chi tiết
            </h2>
...
export default async function AccountDetailPage({ params }: Props) {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-black flex flex-col font-sans selection:bg-[#f58220] selection:text-white">
